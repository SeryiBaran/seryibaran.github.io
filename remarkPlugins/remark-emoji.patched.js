...
const RE_SHORT = / [$@|*'",;.=:\-)([\]\\/<>038BOopPsSdDxXzZ]{2,5}/g; // добавил пробел в начале, потому что в `linux)` считает `x)` за emoji
...
export default function plugin(options) {
  ...
  function getEmoticon(match) {
    ...
    console.log(match);
    const iconFull = emoticon.find(
      (e) => e.emoticons.includes(match.replace(/^ /, "")) // добавил .replace(/^ /, ""), чтобы искало не ` :)` а `:)`
    ); // full match
