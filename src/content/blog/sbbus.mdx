...
Адреса суммируются, т.е. если подключить 8x8 сетку `switches`, макс возможный адрес будет 1024 + 1 (лишний адрес это пустой сортер), но это бесполезно т.к. макс банка памяти на 512 адресов.
