 ...
 * @param step step of adding asterisks on each line. Result of generation with odd `step` will be shit
 * @returns star triangle
 *
 * @example
 * ```ts
 * console.log(generateStarTriangle(10))
 * //          *
 * //         ***
 * //        *****
 * //       *******
 ...
 * ```
 */
function generateStarTriangle(linesNumber: number, step = 2) {
  let lines = [];
  for (let i = 1; i < linesNumber * step; i += step) {
    const line = '*'.repeat(i);

    const leftPadWithStars = Math.floor((line.length + linesNumber * step) / 2);
    lines.push(line.padStart(leftPadWithStars).padEnd(linesNumber));
  }
  return lines.join('\n');
}
```
...
<Alert>Перед отладкой нужно запустить `npm run dev`</Alert>
